...
The command finds its document by the URI in the link rather than using whichever editor is active. Clicking inside a hover widget does not reliably leave the editor beneath it active, and the first version checked `activeTextEditor`, did not match, and returned in silence — which is what "clicking does nothing" looks like from the outside. Where it genuinely cannot act, because the file was closed or the line stopped being an item, it now says so instead of failing quietly.
