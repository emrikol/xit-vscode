...
- Due dates whose period has passed are marked. A due date names a period rather than always a day, so it counts as passed only once the period has ended: `-> 2026` becomes overdue on 1 January 2027, not in March. Weeks follow ISO 8601. Marked with a tinted background and a border rather than a foreground colour, because a decoration's `color` overrides the theme's own colour for the date and an overdue date then stops looking like a date. The border is what carries the contrast: WCAG's 4.5:1 for body text and 3:1 for a non-text indicator cannot both be met by a background wash, and `test/contrast.test.mjs` computes both ratios across the default themes on every commit. Turn it off with `xit.overdueDueDates`; recolour with `xit.overdueDueDateBackground` and `xit.overdueDueDateBorder`. This is the only thing here that a grammar cannot do, because it needs to know what today is; it is drawn as a decoration rather than through semantic tokens, so it adds a rule the grammar lacks instead of restating one it has.
