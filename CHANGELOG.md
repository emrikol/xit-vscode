...
- **`+` is a legal tag value character.** Spec §Tag allows only letters, digits, `_` and `-` unquoted, so `#repeat=+7d` parsed as `#repeat=` with no value at all - silently. Costs no conformance divergence: no example in the corpus has an unquoted `+`.
